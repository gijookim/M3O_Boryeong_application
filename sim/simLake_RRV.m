function [Jflo_Rel, Jflo_Res, Jflo_Vul, Jirr_Rel, Jirr_Res, Jirr_Vul] = simLake_RRV( q, s_in, policy )
...
for tt = 1:size(q_sim)-1
    Jflo_rho(tt) = ((g_flo(tt)==0)&(g_flo(tt+1)>0));
    Jirr_rho(tt) = ((g_irr(tt)==0)&(g_irr(tt+1)>0));
end


Jflo_Rel = 1-sum(g_flo>0)/tt; %alpha
Jflo_Res = (1-Jflo_Rel)/(sum(Jflo_rho)/59);
Jflo_Vul = mean(g_flo);

Jirr_Rel = 1-sum(g_irr>0)/tt; %alpha
Jirr_Res = (1-Jirr_Rel)/(sum(Jirr_rho)/59);
Jirr_Vul = mean(g_irr);
