...
% sys_param.simulation.w     = load('monthly_demand_9819.txt','-ascii') ;    % irrigation demand (m3/s)
% sys_param.simulation.hFLO  = load('monthly_zone2_9819.txt','-ascii') ;     % m the flooding threshold (10^6 m3)
...
%% For K-fold Cross Validation (K=4-1,4-2)
q_ALL = load('monthly_inflow_9919.txt','-ascii');
w_ALL = load('monthly_avg_demand_9919.txt','-ascii') ;    % demand in AVG (m3/s)
star_ALL = load('monthly_avg_zone2_9919.txt','-ascii') ;     % Target Storage in AVG (10^6 m3)

% k=4-1
sys_param.simulation.q     = [q_ALL(1:192);q_ALL(217:240)];
sys_param.simulation.q_aavg= repelem(mean(sys_param.simulation.q),24)' ; 
% sys_param.simulation.q_mavg= load('monthly_avg_k4.txt','-ascii') ; 
sys_param.simulation.q_evl = q_ALL(193:216);
sys_param.simulation.s_in  = 26.222 ;  
sys_param.simulation.w     = [w_ALL(1:192);w_ALL(217:240)];
sys_param.simulation.hFLO  = [star_ALL(1:192);star_ALL(217:240)]; 
sys_param.simulation.r_min = 0;
sys_param.simulation.r_max = 1500;
sys_param.simulation.s_min = 8.18;
sys_param.simulation.s_max = 106.4;
...
% k=4-2
sys_param.simulation.q     = q_ALL(1:216);
sys_param.simulation.q_aavg= repelem(mean(sys_param.simulation.q),24)' ; 
% sys_param.simulation.q_mavg= load('monthly_avg_k4.txt','-ascii') ; 
sys_param.simulation.q_evl = q_ALL(217:240);
sys_param.simulation.s_in  = 40.064;  
sys_param.simulation.w     = [w_ALL(1:192);w_ALL(217:240)];
sys_param.simulation.hFLO  = [star_ALL(1:192);star_ALL(217:240)]; 
sys_param.simulation.r_min = 0;
sys_param.simulation.r_max = 1500;
...
%% For RRV Calculation: ONLY DEMAND
global sys_param;
